...
export default function MusicClient({
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const result = await adminGetMusic();
      if (!result.success) {
        throw new Error(result.error);
      }
      setSongs(result.data);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : '获取音乐列表失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editMode === 'create') {
        const result = await adminCreateMusic({ name, artist, url, cover });
        if (!result.success) {
          throw new Error(result.error);
        }
        ...
        const result = await adminUpdateMusic(selectedSong.id, {
          name,
          artist,
          url,
          cover,
  ...
  const handleDelete = async () => {
    if (!selectedSong) return;
    try {
      const result = await adminDeleteMusic(selectedSong.id);
      if (!result.success) {
        throw new Error(result.error);
      }
    ...
    <>
      <AdminPageLayout
        ...
        actions={[
          ...
          createAddAction(handleCreate, '添加音乐'),
        ]}
      >
