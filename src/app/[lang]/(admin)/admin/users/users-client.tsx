...
function UsersList({
  ...
  onCreateUser: () => void;
  onEditPassword: (user: UserResponse) => void;
      ...
      actions={[
        ...
        ...(isSuperAdmin
          ? [
              ...
              createAddAction(onCreateUser, '创建用户'),
            ]
          : []),
      ]}
    >
...
function CreateUserDialog({
  ...
  setNewRole: (value: 'super_admin' | 'user') => void;
...
function PermissionDialog({
  ...
  open: boolean;
  onOpenChange: (open: boolean) => void;
  editingRole: 'user' | 'super_admin' | null;
  isSubmitting: boolean;
  permissionDefinitions: PermissionGroup[];
...
// === 角色权限编辑（状态 + handlers，封装为独立 hook） ===
function useRolePermissions(setSubmitting: (value: boolean) => void) {
...
export default function UsersClient({
  ...
  // 权限编辑状态（封装为独立 hook，见下方 useRolePermissions）
  const {
    permissionDialogOpen,
    setPermissionDialogOpen,
    permissionDefinitions,
    ...
    handleSavePermissions,
    handleResetPermissions,
  } = useRolePermissions(setIsSubmitting);
  ...
  return (
    <>
      <UsersList
        ...
        onEditPermissions={() => openPermissionDialog('user')}
