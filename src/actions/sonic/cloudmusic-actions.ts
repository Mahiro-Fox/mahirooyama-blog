...
interface PlaylistPayload {
  ...
  playlist?: { id?: string; name?: string; cover?: string; trackCount?: number; loadedCount?: number };
  error?: string;
}
...
export async function neteaseSearchAction(keywords: string, limit = 30) {
  ...
  try {
    const data = await goFetch<CloudSearchPayload>(`/api/cloudmusic/netease/search${query}`);
    return { ok: true, songs: mapSongs(data.songs), rawCount: data.rawCount ?? 0 };
  } catch (e) {
...
export async function neteasePlayableAction(id: string | number, br = '') {
  ...
  try {
    const data = await goFetch<{ url?: string }>(`/api/cloudmusic/netease/playable${query}`);
...
export async function neteaseAccountAction() {
  try {
    const data = await goFetch<{ valid?: boolean; userId?: any; nickname?: string }>(
...
export async function neteaseDailyAction(limit = 30) {
  ...
  try {
    const data = await goFetch<{ songs?: any[] }>(`/api/cloudmusic/netease/daily${query}`);
...
export async function neteasePlaylistAction(id: string | number, limit = 100) {
  ...
  try {
    const data = await goFetch<PlaylistPayload>(`/api/cloudmusic/netease/playlist${query}`);
    return { ok: true, songs: mapSongs(data.songs), totalCount: data.totalCount ?? 0, rawTrackCount: data.rawTrackCount ?? 0 };
  } catch (e) {
    logger.error('neteasePlaylistAction failed', e, { id });
    return { ok: false, errors: (e as Error).message, songs: [], totalCount: 0 };
  }
}
...
export async function neteaseLyricAction(id: string | number) {
  ...
  try {
    ...
    return { ok: true, lyric: data.lyric ?? '', translatedLyric: data.translatedLyric ?? '' };
  } catch (e) {
    logger.error('neteaseLyricAction failed', e, { id });
    return { ok: false, errors: (e as Error).message, lyric: '', translatedLyric: '' };
  }
}
...
export async function qqSearchAction(keywords: string, limit = 12) {
  const query = buildQuery({ keywords, limit: String(limit) });
  try {
    const data = await goFetch<CloudSearchPayload>(`/api/cloudmusic/qq/search${query}`);
    return { ok: true, songs: mapSongs(data.songs), rawCount: data.rawCount ?? 0 };
  } catch (e) {
...
export async function qqSongUrlAction(mid: string, mediaMid = '', quality = '') {
  const query = buildQuery({ mid, mediaMid: mediaMid || undefined, quality: quality || undefined });
  try {
    const data = await goFetch<{ url?: string; playable?: boolean; level?: string; quality?: string; message?: string }>(
      `/api/cloudmusic/qq/songurl${query}`
    );
    return { ok: true, url: data.url ?? '', playable: Boolean(data.playable), level: data.level, quality: data.quality, message: data.message };
  } catch (e) {
    logger.error('qqSongUrlAction failed', e, { mid });
    return { ok: false, errors: (e as Error).message, url: '', playable: false };
  }
}

export async function qqProfileAction() {
  try {
    const data = await goFetch<{ provider?: string; loggedIn?: boolean; userId?: any; nickname?: string; avatar?: string; hasCookie?: boolean; playbackKeyReady?: boolean }>(
...
export async function qqPlaylistTracksAction(id: string | number, limit = 50) {
  const query = buildQuery({ id: String(id), limit: String(limit) });
  try {
    const data = await goFetch<PlaylistPayload>(`/api/cloudmusic/qq/playlist/tracks${query}`);
    return { ok: true, songs: mapSongs(data.songs), playlist: data.playlist, totalCount: data.totalCount ?? data.playlist?.trackCount ?? 0 };
  } catch (e) {
...
export async function qqLyricAction(mid: string, id = '') {
  ...
  try {
    const data = await goFetch<{ lyric?: string; tlyric?: string; qrc?: string; roma?: string }>(
      `/api/cloudmusic/qq/lyric${query}`
    );
    return { ok: true, lyric: data.lyric ?? '', tlyric: data.tlyric ?? '', qrc: data.qrc ?? '', roma: data.roma ?? '' };
  } catch (e) {
...
export async function saveCloudCookie(provider: 'netease' | 'qq', cookie: string) {
  try {
    const data = await goFetch<{ provider?: string; valid?: boolean; loggedIn?: boolean; userId?: any; nickname?: string; saved?: boolean }>(
      '/api/cloudmusic/cookie',
      { method: 'PUT', body: JSON.stringify({ provider, cookie }) }
    );
    return { ok: true, ...data };
  } catch (e) {
...
export async function cloudCookieStatus() {
  try {
    const data = await goFetch<{ netease?: any; qq?: any }>('/api/cloudmusic/cookie');
    return { ok: true, ...data };
  } catch (e) {
...
export async function resolveSongPlayback(
  ...
  if (provider === 'qq') {
    ...
    return { provider, url: urlRes.url, lyric: lyricRes.lyric, tlyric: lyricRes.tlyric, ok: urlRes.ok };
  }
  const [urlRes, lyricRes] = await Promise.all([
    neteasePlayableAction(song.id),
    neteaseLyricAction(song.id),
  ]);
  return { provider, url: urlRes.url, lyric: lyricRes.lyric, translatedLyric: lyricRes.translatedLyric, ok: urlRes.ok };
}