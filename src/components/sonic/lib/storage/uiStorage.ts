/**
 * UI 状态本地存储。
 * 持久化本地歌单、置顶的云端歌单、搜索来源偏好与侧栏提示是否已读等界面状态。
 */
...
const PINNED_NETEASE_PLAYLISTS_STORAGE_KEY =
  'sonic-topography-pinned-netease-v1';
...
export function writePinnedNeteasePlaylistsStorage(
  pinned: string[],
  storage?: Storage
) {
...
export function writePinnedQQPlaylistsStorage(
  pinned: string[],
  storage?: Storage
) {
...
export function readSearchProviderStorage(storage?: Storage): SearchProvider {
  return getStorage(storage)?.getItem(SEARCH_PROVIDER_STORAGE_KEY) === 'qq'
    ? 'qq'
    : 'netease';
}

export function writeSearchProviderStorage(
