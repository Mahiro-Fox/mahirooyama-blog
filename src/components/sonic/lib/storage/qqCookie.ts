/**
 * QQ 音乐登录凭证。
 * 负责 Cookie 与 Uin 的规范化、登录状态判断、本地存取与请求头构建。
 */
...
export function normalizeQQCookie(value: string | null | undefined) {
  ...
  if (Number(cookie.login_type) === 2 && cookie.wxuin && !cookie.uin)
    cookie.uin = cookie.wxuin;
  if (!cookie.uin && (cookie.qqmusic_uin || cookie.p_uin))
    cookie.uin = cookie.qqmusic_uin || cookie.p_uin;
...
export function createQQCookieHeaders(
...
export function writeQQCookieStorage(cookie: string) {
  ...
  const normalized = normalizeQQCookie(cookie);
  if (normalized)
    window.localStorage.setItem(QQ_COOKIE_STORAGE_KEY, normalized);
