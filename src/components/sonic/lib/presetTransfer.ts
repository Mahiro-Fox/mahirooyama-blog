...
export function normalizePresetTransferPackage(value: unknown): PresetTransferPackage {
  ...
  const availableThemeIds = [...BUILT_IN_THEME_IDS, ...customThemes.map((preset: CustomThemeSettings) => preset.id)];
...
export function createPresetTransferPackage(options: CreatePresetTransferOptions = {}): PresetTransferPackage {
  const customThemes = (readJsonStorage(CUSTOM_THEME_STORAGE_KEY) as unknown[] | undefined)?.map((preset: any) => normalizeCustomThemeSettings(preset))
    || [defaultCustomThemeSettings];
