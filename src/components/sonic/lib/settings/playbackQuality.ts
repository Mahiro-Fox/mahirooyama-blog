/**
 * 播放音质设置。
 * 定义网易云码率与 QQ 音质等级选项、默认值与持久化，并据此构建各平台的播放地址。
 */
...
const QQ_QUALITY_VALUES = new Set<QQPlaybackQuality>([
  'lossless',
  'exhigh',
  'standard',
  'aac',
]);
const NETEASE_BITRATE_VALUES = new Set<NeteasePlaybackBitrate>([
  '320000',
  '192000',
  '128000',
]);

export const QQ_PLAYBACK_QUALITY_OPTIONS: Array<{
  value: QQPlaybackQuality;
  label: string;
}> = [
...
export const NETEASE_PLAYBACK_BITRATE_OPTIONS: Array<{
  value: NeteasePlaybackBitrate;
  label: string;
}> = [
...
export function normalizePlaybackQualitySettings(
  value: unknown
): PlaybackQualitySettings {
  const input =
    value && typeof value === 'object'
      ? (value as Partial<PlaybackQualitySettings>)
      : {};
  ...
  const neteaseBitrate = NETEASE_BITRATE_VALUES.has(
...
export function writePlaybackQualitySettingsStorage(
