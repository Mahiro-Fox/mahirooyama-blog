...
import type { CloudPlaylistSummary as NeteasePlaylistSummary } from '../types';
import {
  readSavedPlaylists,
  writeSavedPlaylists,
} from './uiStorage';
...
export async function loadCloudPayload<T = SongListPayload>(
  ...
  if (path.endsWith('/netease/playlists') || provider === 'netease' && url.includes('/netease/playlists')) {
    const res = await neteasePlaylistsAction();
    return res.ok
      ? asApiResponse({ playlists: toPlaylistSummary(res.playlists) } as unknown as T)
      : failedResponse<T>({ error: res.errors } as unknown as T);
  }
  if (path.endsWith('/qq/user/playlists') || (provider === 'qq' && url.includes('/qq/user/playlists'))) {
  ...
  if (path.includes('/qq/playlist/tracks') && id) {
    ...
    return res.ok
      ? asApiResponse({ songs: toSongList(res.songs), totalCount: res.totalCount, playlist: res.playlist } as unknown as T)
      : failedResponse<T>({ error: res.errors } as unknown as T);
  }
  if (path.includes('/netease/playlist') && id) {
    const res = await neteasePlaylistAction(id, 100);
    return res.ok
      ? asApiResponse({ songs: toSongList(res.songs), totalCount: res.totalCount } as unknown as T)
      : failedResponse<T>({ error: res.errors } as unknown as T);
  }
  return failedResponse<T>({ error: `Unknown cloud endpoint: ${url}` } as unknown as T);
}

// ── 本地歌单持久化（浏览器 localStorage），替代 /api/playlists ──
export async function loadServerPlaylists(): Promise<ApiResponse<{ playlists?: SavedPlaylist[] }>> {
...
export async function syncNeteaseProxyCookie(cookie: string): Promise<ApiResponse<{ valid?: boolean }>> {
...
export async function syncQQProxyCookie(cookie: string): Promise<ApiResponse<{ loggedIn?: boolean }>> {
...
export async function loadSongLyrics(
  ...
  _qqCookie: string,
): Promise<ApiResponse<{ lyric?: string; translatedLyric?: string; tlyric?: string; qrc?: string }>> {
...
export async function loadSongPlaybackResources(
  ...
  if (provider === 'qq') {
    ...
    const [playback, lyricData] = await Promise.all([
      ...
      loadSongLyrics(song, '', '') as Promise<ApiResponse<{ lyric?: string; translatedLyric?: string; tlyric?: string; qrc?: string }>>,
    ]);
    return {
      provider,
      urlData: { url: playback.url, message: playback.message, ok: playback.ok },
