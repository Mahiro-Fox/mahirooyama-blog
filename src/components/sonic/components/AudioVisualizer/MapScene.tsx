/**
 * 3D 音频可视化主场景。
 * 基于 React Three Fiber 渲染地形地图、浮动方块、封面屏幕与空间歌词，
 * 并把音频频谱与节拍数据映射到地形起伏、颜色与相机运动上。
 */
...
} from '../../lib/scene/terrainResponse';
import {
...
import { themes, type ThemeColors } from '../../lib/theme/themes';
import {
  CoverShaderMaterial,
 ...
 * 3D 音频可视化主场景组件。
 *
 * @param themeColors 当前主题色板，驱动场景配色
 * @param groundEqSettings 地面均衡器设置（8 段增益、振幅、地形密度等）
 * @param rotationSpeed 场景自动旋转速度
 * @param coverUrl 当前曲目封面地址，用于场景中的封面屏幕
 * @param lyricsText 当前歌词原文（LRC 文本）
 * @param lyricsSettings 歌词样式设置（位置/字体/每行字数等）
 * @param lyricsVisible 是否显示歌词
 * @param isPerspectiveEditMode 是否处于透视编辑（相机调整）模式
 * @param resetCameraTrigger 自增触发器，变化时重置相机
 */
export function MapScene({
  ...
  useFrame((state, delta) => {
    ...
    const motionSpeed = Math.max(
      0,
      Math.min(100, groundEqSettings.motionSpeed ?? DEFAULT_GROUND_MOTION_SPEED)
    );
