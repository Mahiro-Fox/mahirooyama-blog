...
/**
 * 开屏动画：启动时播放，结束后回调进入主界面。
 *
 * @param onComplete 动画（含渐出）结束后的回调
 * @param duration 开屏展示时长（毫秒），之后淡出
 * @param surfaceColor 页面背景色
 * @param accentColor 页面强调色
 */
export const SplashScreen: React.FC<SplashScreenProps> = ({
  ...
  useEffect(() => {
    ...
    const surfaceColorUniform = gl.getUniformLocation(
      program,
      'u_surfaceColor'
    );
    ...
    const render = (time: number) => {
      ...
      const sColor = surfaceColor
        ? parseHex(surfaceColor)
        : [0.031, 0.043, 0.075];
      const aColor = accentColor ? parseHex(accentColor) : [0.09, 0.17, 0.3];
