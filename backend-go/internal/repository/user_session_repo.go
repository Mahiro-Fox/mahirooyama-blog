// Package repository
// user_session_repo.go：前台访客会话 CRUD（与 admin_session_repo 同结构，不做接口抽象——
// 项目约定不使用 OOP；保持两份独立实现，方便后续各自演进）
package repository
...
func CreateUserSession(ctx context.Context, db *gorm.DB, s *model.UserSession) error {
...
func GetUserSessionByToken(ctx context.Context, db *gorm.DB, token string) (*model.UserSession, error) {
...
func DeleteUserSessionByToken(ctx context.Context, db *gorm.DB, token string) error {
	res := db.WithContext(ctx).Where("token = ?", token).Delete(&model.UserSession{})
	if res.Error != nil {
...
func DeleteUserSessionsByAccountID(ctx context.Context, db *gorm.DB, accountID string) (int64, error) {
	res := db.WithContext(ctx).Where("account_id = ?", accountID).Delete(&model.UserSession{})
	if res.Error != nil {
...
func UpdateUserSessionLastUsedAt(ctx context.Context, db *gorm.DB, token string, at time.Time) error {
...
func DeleteExpiredUserSessions(ctx context.Context, db *gorm.DB, now time.Time) (int64, error) {
	res := db.WithContext(ctx).
